...
def select_data():
    ...
    try:
        # 打开数据库连接
        conn = pymysql.connect(host='127.0.0.1', port=3306, user='root', passwd='qq1968904856', db='doubanmovie',
                               charset='utf8')
        # 使用cursor方法创建一个游标
        cursor = conn.cursor()
        # 查询数据表数据
        # 查询统计不同上映年份的电影数量
        sql = "select distinct showtime,count(showtime) from movie_info  where showtime is not null group by showtime order by showtime "
        cursor.execute(sql)
        # 年份、数量集合
        ...
        print(total_num)
        # 查询中国上映年份
        sql2 = "select showtime,count(showtime) as num from movie_info where showtime is not null and  areas like '%中国%' group by showtime order by showtime"
        cursor.execute(sql2)
        ...
        # 查询美国上映年份
        sql3 = "select showtime,count(showtime) as num from movie_info where showtime is not null and areas  like '%美国%' group by showtime order by showtime"
        cursor.execute(sql3)
...
def show_showtime():
    ...
    line.set_global_opts(
        ...
        title_opts=opts.TitleOpts(
            title="上映年份分布图(可拖动下方滑条互动)", pos_left='center'),
        # 标签
