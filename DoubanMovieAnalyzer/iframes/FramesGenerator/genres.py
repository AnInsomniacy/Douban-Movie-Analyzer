...
def select_data():
    ...
    try:
        ...
        # 查询数据表数据
        sql = "select genres,count(genres) from movie_info where genres is not null group by genres"
        cursor.execute(sql)
        # 取出每部电影的类型集合
...
def show_genres():
    data = select_data()
    # 矩形树图初始化
    treemap = TreeMap(init_opts=opts.InitOpts(page_title="电影类型分布"))
    # 添加数据
    treemap.add(
        series_name='电影类型',
        data=data,
    ...
    treemap.set_global_opts(
        ...
        title_opts=opts.TitleOpts(
            title="电影类型分布(可点击卡片互动)", pos_left='center'
        ),
        # 标签隐藏
        legend_opts=opts.LegendOpts(is_show=False)
    )
    # 生成HTML
    html = "iframes/genres.html"
