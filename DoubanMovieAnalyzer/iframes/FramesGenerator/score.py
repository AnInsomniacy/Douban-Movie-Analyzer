...
def select_data():
    ...
    try:
        ...
        # 查询数据表数据
        # 查询评分
        sql = "select score,count(score) from movie_info where score is not null group by score order by score "
        cursor.execute(sql)
        ...
        # 查询中国评分
        sql = "select score,count('score') as num from movie_info where score is not null and areas like '%中国%'group by score order by score "
        cursor.execute(sql)
        rows = cursor.fetchall()
        for i in rows:
            china_dict[i[0]] = i[1]
        ...
        # 查询美国国评分
        sql = "select score,count('score') as num from movie_info where score is not null and areas like '%美国%'group by score order by score "
        cursor.execute(sql)
        rows = cursor.fetchall()
        for i in rows:
...
def show_score():
    ...
    line.set_global_opts(
        title_opts=opts.TitleOpts(title="中外电影评分分布(拖动下方滑条可互动)", pos_left='center'),
        # 水平滚动条
    ...
    # 生成HTML
    html = "iframes/dynamic_graph_web.html"
